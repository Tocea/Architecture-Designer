...
public class ArchitectureDSLJavaValidator extends AbstractArchitectureDSLJavaValidator {
	...
	public void checkSuperPattern(Pattern pattern) {
		if(pattern.getSuperPattern()!=null) {
			if(!(pattern.getSuperPattern().getReference() instanceof Pattern))
				error("A pattern can only extend a pattern.", ArchitectureDSLPackage.Literals.PATTERN__SUPER_PATTERN);
			if(!checkInheritanceCycle(pattern))
				error(CYCLE_DETECTED, ArchitectureDSLPackage.Literals.PATTERN__SUPER_PATTERN);
		}
	}
	...
	public void checkSuperRole(Role role) {
		if(role.getSuperRole()!=null) {
			if(!(role.getSuperRole().getReference() instanceof Role))
				error("A role can only extend a role", ArchitectureDSLPackage.Literals.ROLE__SUPER_ROLE);
			if(!checkInheritanceCycle(role))
				error(CYCLE_DETECTED, ArchitectureDSLPackage.Literals.ROLE__SUPER_ROLE);
		}
	}
		...
		for(ExtensionEntity entity : extension.getEntities())
			if(entity instanceof NamedEntity) {
				...
				if(entities.containsKey(name)) {
					error(DUPLICATE_NAME+name, namedEntity, ArchitectureDSLPackage.Literals.NAMED_ENTITY__NAME, null);
					error(DUPLICATE_NAME+name, entities.get(name), ArchitectureDSLPackage.Literals.NAMED_ENTITY__NAME, null);
				}
				else
	...
	public void checkParameterUniqueName(ParametrizedType type) {
		...
		for(Parameter parameter : type.getParameters()) {
			...
			if(parameters.containsKey(name)) {
				error(DUPLICATE_NAME+name, parameter, ArchitectureDSLPackage.Literals.NAMED_ENTITY__NAME, null);
				error(DUPLICATE_NAME+name, parameters.get(name), ArchitectureDSLPackage.Literals.NAMED_ENTITY__NAME, null);
			}
			else
		...
		for(ExtensionEntity entity : extension.getEntities())
			if(entity instanceof Datatype) {
				...
				if(datatypes.containsKey(name)) {
					error(DUPLICATE_NAME+name, datatype , ArchitectureDSLPackage.Literals.DATATYPE__REFERENCE, null);
					error(DUPLICATE_NAME+name, datatypes.get(name) , ArchitectureDSLPackage.Literals.DATATYPE__REFERENCE, null);
				}
				else
	...
	public void checkArity(Field field) {
		if(field.getLb()!=null && field.getUb()!=null &&
			...
			error("Lower bound must be lower than upper bound.", ArchitectureDSLPackage.Literals.FIELD__LB);
	}
	...
	private boolean checkInheritanceCycle(Pattern pattern) {
		try {
			Pattern superPattern = (Pattern) pattern.getSuperPattern().getReference();
			while(superPattern!=null) {
				if(superPattern==pattern)
					return false;
				superPattern = (Pattern) superPattern.getSuperPattern().getReference();
			}
			return true;
		} catch(ClassCastException e) {
			return true;
