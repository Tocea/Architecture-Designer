...
public class XadlTest {
	...
	public void test() throws ClassNotFoundException {
		...
		final TestHarness testHarness = TestAutomatic.newTestHarness(TestAutomatic.getAvailablePlugins());
	...
	private static void classes(final EPackage p, final ClassScanner scanner) throws ClassNotFoundException {

		final ClassLoader classLoader = p.getClass().getClassLoader();

		for (final EClassifier classifier : p.getEClassifiers()) {
			final Class<?> _class = classLoader.loadClass(classifier.getInstanceClassName());
